<g?php require_once __DIR__ . '/../includes/config.php' ; require_once __DIR__ . '/../includes/db.php' ; require_once
    __DIR__ . '/../includes/functions.php' ; if (session_status()===PHP_SESSION_NONE) { session_start(); } if
    (!isset($_GET['id']) || !is_numeric($_GET['id'])) { die("Evento não encontrado."); } $eventId=intval($_GET['id']);
    $evento=getEventDetails($eventId); if (!$evento) { die("Evento não encontrado ou acesso restrito."); }
    $relatedEvents=getRelatedEvents($eventId); $isInscrito=false; if (isset($_SESSION['user_id'])) {
    $usuario_id=$_SESSION['user_id']; $stmt=$pdo->prepare("SELECT COUNT(*) FROM inscricoes WHERE usuario_id =
    :usuario_id AND evento_id = :evento_id");
            ...
            <div class="container">
                <div class="evento-container">
                    ...
                    <?php if (!empty($evento['imagem_capa'])): ?>
                        <img src="<?php echo PASTA_BASE . htmlspecialchars($evento['imagem_capa']); ?>"
                            alt="Imagem do evento" class="capa_evento_single">
                    <?php endif; ?>
                    ...
                    <!-- Realizadores -->
                    <h3 class="subtitulo_evento_infos">Organizadores</h3>
